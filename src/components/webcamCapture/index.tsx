...
const WebcamActionButton = styled.button`
  width:100px;
  bottom: 20px;
  background-color: #fff;
  color: #333;
...
interface WebcamCaptureProps{
    getProfileImage:(iimage:string)=>void
}

const WebcamCapture = ({getProfileImage}:WebcamCaptureProps) => {
    ...
    <WebcamContainer>
      {capturedImage ? (
        <>
          ...
          <div style={{flexDirection:'row',display:'flex',justifyContent:'space-evenly',marginTop:'1rem'}}>
          <WebcamActionButton onClick={()=>resetState()}>Reset</WebcamActionButton>
          <WebcamActionButton onClick={()=>getProfileImage(capturedImage)}>ok</WebcamActionButton>
          </div>
        </>
      ) : (
        <>
